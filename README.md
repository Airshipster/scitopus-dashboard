...
Race использует общий паттерн bar chart race, известный по D3/Observable-визуализациям. В этой версии логика, интерфейс, данные, анимации, переключатели, темы, дополнительные режимы и поведение шкал реализованы как кастомный SciTopus Dashboard, а не как прямой форк стороннего проекта.
...
Дашборд использует D3.js v7.9.0 как runtime-библиотеку для визуализации данных. Подробные сведения о сторонних компонентах и лицензиях находятся в [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md).

Режим Race относится к семейству bar chart race-визуализаций. В текущих файлах проекта не найдено прямой ссылки на конкретный исходный GitHub-репозиторий, поэтому репозиторий оформлен не как fork, а как самостоятельный проект с указанием использованной библиотеки и визуального паттерна. Если будет найден точный исходный репозиторий или конкретный фрагмент кода, его нужно добавить в notices отдельной строкой с автором, ссылкой и лицензией.
